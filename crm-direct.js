...
const SUPABASE_KEY='eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InZ0bnVjZXZoeWhmdnltcHBqYmZhIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODU1MDkxMTUsImV4cCI6MjEwMTA4NTExNX0.nnQHMvp_G508Lkeza0JktA-wzKwXu54S-PKw8i0D4pg';
...
async function rpc(name,args={}){
  ...
  if(!response.ok){
    const message=payload?.message||payload?.details||payload?.hint||raw||`HTTP ${response.status}`;
...
async function loadAll(){
  setStatus('Оновлення стану бази…');
  ...
  try{
    ...
    state.sellers=Array.isArray(sellers)?sellers:[];
    renderOverview();
    renderCities();
    renderUnits();
    renderDuplicates();
    renderSellers();
    renderInviteUnits();
    setStatus(`Синхронізовано ${new Date().toLocaleString('uk-UA')} · ${state.units.length} записів`,'success');
    ...
    $('kpiGrid').innerHTML=`<button class="kpi alert" id="retryLoad"><strong>!</strong><span>Натисніть, щоб повторити</span></button>`;
...
function renderOverview(){
  ...
  const cards=[
    ['На сайті',t.on_site,''],['Published',t.published,''],['Review',t.review,'warn'],['Archive',t.archive,''],
    ['Можливі дублі',t.duplicates,'alert'],['Недоступні',t.unavailable,'alert'],['Застарілі',t.stale,'warn'],['Джерела',t.sources,'']
  ];
  ...
  $('catalogBadge').textContent=t.units||state.units.length;
  $('duplicateBadge').textContent=t.duplicates||0;
  $('sellerBadge').textContent=state.sellers.length;
  $('kpiGrid').querySelectorAll('[data-kpi]').forEach(b=>b.onclick=()=>{
    const map={'На сайті':'on_site','Published':'published','Review':'review','Archive':'archive','Можливі дублі':'duplicates','Недоступні':'unavailable','Застарілі':'stale'};
    const next=map[b.dataset.kpi];
    if(!next)return;
    state.scope=next;
...
function renderCities(){
  const rows=Array.isArray(state.overview?.cities)?state.overview.cities:[];
  const t=state.overview?.totals||{};
  const all=`<button class="city-card ${state.city==='all'?'active':''}" data-city="all"><strong>Усі міста</strong><small>${t.units||state.units.length} записів</small><div class="city-numbers"><span>${t.on_site||0} сайт</span><span>${t.review||0} review</span></div></button>`;
  $('cityStrip').innerHTML=all+rows.map(c=>`<button class="city-card ${state.city===c.city?'active':''}" data-city="${esc(c.city)}"><strong>${esc(c.city)}</strong><small>${c.total||0} записів</small><div class="city-numbers"><span>${c.on_site||0} сайт</span><span>${c.review||0} review</span>${c.duplicates?`<span>${c.duplicates} дуб.</span>`:''}</div></button>`).join('');
  $('cityHint').textContent=state.city==='all'?'Оберіть місто':`Фільтр: ${state.city}`;
  $('cityStrip').querySelectorAll('[data-city]').forEach(b=>b.onclick=()=>{
    state.city=b.dataset.city;
    state.sellerUnitFilter=null;
    renderCities();
    renderUnits();
  });
}
...
function filteredUnits(){
  ...
  rows.sort((a,b)=>{
    ...
    return priority(a)-priority(b)||new Date(b.updated_at||0)-new Date(a.updated_at||0);
  });
  return rows;
}
...
function renderUnits(){
  ...
  $('unitEmpty').hidden=rows.length>0;
  $('listTitle').textContent=state.sellerUnitFilter?'Об’єкти продавця':state.city==='all'?'Усі записи':state.city;
  $('unitList').innerHTML=rows.map(u=>`<button class="unit-item ${state.activeId===u.unit_id?'active':''}" data-unit="${u.unit_id}">${u.cover_photo_url?`<img loading="lazy" src="${esc(u.cover_photo_url)}" alt="">`:'<span class="unit-photo-placeholder">⌂</span>'}<span><h3>${esc(u.title||'Без назви')}</h3><p>${esc(u.city||'Без міста')}${u.address?' · '+esc(u.address):''}</p><span class="unit-meta">${badge(u.publication||'—',u.publication||'')}${u.on_site?badge('на сайті','site'):''}${u.review_status?badge(u.review_status,u.review_status):''}${u.possible_duplicate_of?badge('possible duplicate','duplicate'):''}<span class="badge">${u.photo_count||0} фото</span><span class="badge">${u.source_count||0} дж.</span><span class="badge">${u.seller_count||0} прод.</span></span></span><span class="unit-price">${money(u.price,u.currency)}<br><small>${u.area_m2?u.area_m2+' м²':''}</small></span></button>`).join('');
...
async function openUnit(id,scroll=true){
  state.activeId=id;
  renderUnits();
  $('unitDetail').innerHTML='<div class="empty">Завантаження картки…</div>';
  try{
    state.workspace=await rpc('admin_unit_workspace',{p_unit_id:id});
    renderDetail();
    if(scroll&&innerWidth<850)$('unitDetail').scrollIntoView({behavior:'smooth',block:'start'});
  }catch(error){
    $('unitDetail').innerHTML=`<p class="error">${esc(error.message)}</p>`;
  }
}
...
function renderSummary(u){
  ...
  const f=$('unitForm');
  f.review_status.value=u.review_status||'in_progress';
  f.source_access_status.value=u.source_access_status||'not_checked';
  $('saveReview').onclick=()=>saveUnit(f.review_status.value);
  $('publishReview').onclick=()=>saveUnit('verified');
  $('archiveReview').onclick=()=>saveUnit('retired');
}
...
async function saveUnit(status){
  ...
  const unresolved=f.unresolved.value.split(',').map(x=>x.trim()).filter(Boolean);
  st.textContent='Збереження…';st.className='';
  try{
    await rpc('admin_save_listing_review',{p_unit_id:state.activeId,p_patch:patch,p_review_status:status,p_source_access_status:f.source_access_status.value,p_unresolved_fields:unresolved,p_notes:f.notes.value});
    st.textContent='Збережено';st.className='success';
    await loadAll();
  }catch(error){st.textContent=error.message;st.className='error'}
}
...
function renderDuplicateDetail(w){
  ...
  $('markDuplicate').onclick=async()=>{
    const target=$('duplicateTarget').value;
    if(!target)return alert('Оберіть пов’язану картку');
    try{await rpc('admin_set_possible_duplicate',{p_unit_id:state.activeId,p_target_unit_id:target,p_reason:$('duplicateReason').value});await loadAll()}catch(error){alert(error.message)}
  };
  $('clearDuplicate').onclick=async()=>{try{await rpc('admin_set_possible_duplicate',{p_unit_id:state.activeId,p_target_unit_id:null,p_reason:null});await loadAll()}catch(error){alert(error.message)}};
}

function renderPhotos(w){
  $('detailPhotos').innerHTML=(w.photos||[]).length?`<div class="photo-grid">${w.photos.map(p=>p.url?`<a href="${esc(p.url)}" target="_blank"><img loading="lazy" src="${esc(p.url)}" alt=""></a>`:'').join('')}</div>`:'<div class="empty">Фотографій немає.</div>';
}

function renderDuplicates(){
  const rows=state.units.filter(u=>u.possible_duplicate_of);
  $('duplicateEmpty').hidden=rows.length>0;
  $('duplicateSummary').textContent=`${rows.length} зв’язків потребують рішення`;
...
function renderSellers(){
  const q=norm($('sellerSearch')?.value);
  const rows=state.sellers.filter(s=>!q||norm([s.display_name,s.phone,s.email,s.platform].join(' ')).includes(q));
  $('sellerEmpty').hidden=rows.length>0;
  ...
  $('sellerDirectory').querySelectorAll('[data-seller-units]').forEach(b=>b.onclick=()=>{
    state.sellerUnitFilter=b.dataset.sellerUnits.split(',').filter(Boolean);
    state.city='all';state.scope='all';$('scopeFilter').value='all';
    setTab('catalog');renderCities();renderUnits();
  });
}

function renderInviteUnits(){
  const q=norm($('inviteSearch')?.value);
  const rows=state.units.filter(u=>u.on_site&&(!q||norm([u.title,u.city,u.address].join(' ')).includes(q)));
  ...
  $('inviteUnits').querySelectorAll('[data-invite-unit]').forEach(c=>c.onchange=()=>{
    c.checked?state.selectedInvite.add(c.dataset.inviteUnit):state.selectedInvite.delete(c.dataset.inviteUnit);
    $('selectedCount').textContent=state.selectedInvite.size;
  });
  $('selectedCount').textContent=state.selectedInvite.size;
}
...
async function loadInviteHistory(){
  try{
    const rows=await rpc('admin_seller_invites');
    $('inviteHistory').innerHTML=(rows||[]).map(i=>`<div class="history-row"><span>${esc(i.email)}</span><span>${i.unit_count} об.</span><span>${badge(i.status,i.status)}</span><span>${date(i.created_at)}</span><span>${i.status==='pending'?`<button class="secondary" data-revoke="${i.id}">Відкликати</button>`:''}</span></div>`).join('')||'<div class="empty">Запрошень поки немає.</div>';
    $('inviteHistory').querySelectorAll('[data-revoke]').forEach(b=>b.onclick=async()=>{if(!confirm('Відкликати запрошення?'))return;await rpc('admin_revoke_seller_invite',{p_invite_id:b.dataset.revoke});await loadInviteHistory()});
  }catch(error){$('inviteHistory').innerHTML=`<p class="error">${esc(error.message)}</p>`}
}

async function createInvite(){
  const st=$('inviteStatus'),email=$('sellerEmail').value.trim();
  st.textContent='Створення…';
  try{
    const data=await rpc('admin_create_seller_invite',{p_email:email,p_unit_ids:[...state.selectedInvite],p_expires_days:Number($('days').value)||14});
    const url=new URL('./cabinet.html',location.href);url.searchParams.set('invite',data.token);
    $('inviteUrl').value=url.href;$('inviteResult').hidden=false;
    st.textContent=`Готово: ${data.units} оголошень`;st.className='success';
...
function bind(){
  ...
  $('inviteSearch').oninput=renderInviteUnits;
  $('createInvite').onclick=createInvite;
  $('refreshInvites').onclick=loadInviteHistory;
  $('copyInvite').onclick=async()=>{await navigator.clipboard.writeText($('inviteUrl').value);$('copyInvite').textContent='Скопійовано ✓'};
}
