...
const db=createClient('https://vtnucevhyhfvymppjbfa.supabase.co','sb_publishable_-5iiF1Qgq--BKgXLHREINA_nPC7yVVY',{auth:{persistSession:true,autoRefreshToken:true,detectSessionInUrl:true}});
...
const money=(price,currency)=>price?new Intl.NumberFormat('uk-UA',{maximumFractionDigits:0}).format(Number(price))+' '+({USD:'$',EUR:'€',UAH:'₴'}[currency]||currency||''):'Ціна не вказана';
...
function setTab(name){document.querySelectorAll('[data-tab]').forEach(b=>b.classList.toggle('active',b.dataset.tab===name));document.querySelectorAll('.tab-panel').forEach(p=>p.classList.toggle('active',p.id===name+'Panel'))}

async function rpc(name,args={}){const{data,error}=await db.rpc(name,args);if(error)throw error;return data}

async function loadAll(){
  $('syncStatus').textContent='Оновлення стану бази…';
  try{
  ...
  }catch(e){$('syncStatus').textContent=e.message;$('syncStatus').className='error'}
}
...
function renderOverview(){
  ...
  $('kpiGrid').querySelectorAll('[data-kpi]').forEach(b=>b.onclick=()=>{const map={'На сайті':'on_site','Published':'published','Review':'review','Archive':'archive','Можливі дублі':'duplicates','Недоступні':'unavailable','Застарілі':'stale'};if(map[b.dataset.kpi]){state.scope=map[b.dataset.kpi];$('scopeFilter').value=state.scope;setTab('catalog');renderUnits()}});
}

function renderCities(){
  const rows=state.overview?.cities||[];
  const total=state.overview?.totals?.units||0;
...
function filteredUnits(){
  ...
  let rows=state.units.filter(u=>{
    ...
    switch(state.scope){
      case'on_site':return u.on_site;
      case'published':return u.publication==='published';
      case'review':return u.publication==='review';
      case'archive':return u.publication==='archive';
      case'duplicates':return !!u.possible_duplicate_of;
      case'unavailable':return['blocked','removed'].includes(u.source_access_status);
      case'stale':return stale(u.source_checked_at||u.checked_at);
      case'no_photos':return !u.photo_count;
      case'no_price':return !u.price;
      default:return true;
    }
  });
...
function activateDetail(name){state.detailTab=name;$('unitDetail').querySelectorAll('[data-detail]').forEach(b=>b.classList.toggle('active',b.dataset.detail===name));$('unitDetail').querySelectorAll('.detail-section').forEach(s=>s.classList.toggle('active',s.id==='detail'+name[0].toUpperCase()+name.slice(1)))}
...
async function saveUnit(status){
  const f=$('unitForm'),st=$('detailStatus');const patch={};['title','price','currency','area_m2','land_area_sotka','floors','bedrooms','address','source_url','description'].forEach(k=>patch[k]=f.elements[k].value.trim());const unresolved=f.unresolved.value.split(',').map(x=>x.trim()).filter(Boolean);
...
function renderDuplicateDetail(w){
  const current=w.duplicate_target;const sims=w.similar_candidates||[];const reverse=w.reverse_duplicates||[];
  ...
  $('detailDuplicates').querySelectorAll('[data-open-related]').forEach(b=>b.onclick=()=>openUnit(b.dataset.openRelated));$('markDuplicate').onclick=async()=>{const target=$('duplicateTarget').value;if(!target)return alert('Оберіть пов’язану картку');try{await rpc('admin_set_possible_duplicate',{p_unit_id:state.activeId,p_target_unit_id:target,p_reason:$('duplicateReason').value});await loadAll()}catch(e){alert(e.message)}};$('clearDuplicate').onclick=async()=>{try{await rpc('admin_set_possible_duplicate',{p_unit_id:state.activeId,p_target_unit_id:null,p_reason:null});await loadAll()}catch(e){alert(e.message)}};
}
...
function renderSellers(){
  const q=norm($('sellerSearch')?.value);const rows=state.sellers.filter(s=>!q||norm([s.display_name,s.phone,s.email,s.platform].join(' ')).includes(q));$('sellerEmpty').hidden=rows.length>0;
...
function renderInviteUnits(){
  const q=norm($('inviteSearch')?.value);const rows=state.units.filter(u=>u.on_site&&(!q||norm([u.title,u.city,u.address].join(' ')).includes(q)));
...
async function boot(){
  const{data:{session}}=await db.auth.getSession();$('auth').hidden=!!session;$('crm').hidden=!session;if(!session)return;
  try{const isAdmin=await rpc('is_site_admin');if(!isAdmin)throw new Error('Цей акаунт не має адміністративного доступу.');$('auth').hidden=true;$('crm').hidden=false;await loadAll();await loadInviteHistory()}catch(e){$('crm').hidden=true;$('auth').hidden=false;$('authStatus').textContent=e.message;$('authStatus').className='error'}
}

$('login').onsubmit=async e=>{e.preventDefault();const{error}=await db.auth.signInWithOtp({email:$('email').value.trim(),options:{emailRedirectTo:location.href}});$('authStatus').textContent=error?error.message:'Посилання надіслано на email.';$('authStatus').className=error?'error':'success'};
$('logout').onclick=async()=>{await db.auth.signOut();location.reload()};$('refreshAll').onclick=loadAll;
document.querySelectorAll('[data-tab]').forEach(b=>b.onclick=()=>{setTab(b.dataset.tab);if(b.dataset.tab==='invites')loadInviteHistory()});
$('catalogSearch').oninput=e=>{state.query=e.target.value;renderUnits()};$('scopeFilter').onchange=e=>{state.scope=e.target.value;state.sellerUnitFilter=null;renderUnits()};$('sortFilter').onchange=e=>{state.sort=e.target.value;renderUnits()};
